...
function generateGroupedCardsHTML(groupedData) {
    ...
    groupedData.forEach(day => {
        ...
        if (isDayComplete) cardBorderClass = "ring-2 ring-emerald-500 ring-offset-2 ring-offset-slate-900 bg-slate-800";
        else if (isToday) cardBorderClass = "ring-2 ring-blue-500 ring-offset-2 ring-offset-slate-900 bg-slate-800";
        ...
        const rowsHtml = day.workouts.map(w => {
            ...
            if (w.status === 'COMPLETED' || w.status === 'UNPLANNED') statusColorClass = "text-emerald-400";
            else if (w.status === 'MISSED') statusColorClass = "text-red-400";

            // Sport Text Color Class (e.g., text-bike, text-run)
            const sportTextClass = `text-${sportType.toLowerCase()}`;
            ...
            return `
                <div class="flex items-start gap-4 p-4 border-b border-slate-700/50 last:border-0 hover:bg-slate-700/50 transition-colors">
                    ...
                    <div class="flex-1 min-w-0 pt-0.5">
                        <div class="flex items-start gap-2 mb-2 ${sportTextClass}">
