...
// --- STRICT FIELD MAPPING (Matches your JSON exactly) ---
const KEYS = {
    hr: 'averageHR',
    spd: 'averageSpeed',
    pwr: 'avgPower',
...
export const normalizeMetricsData = (rawData) => {
    ...
    return rawData.map(d => ({
        ...
        // Normalized properties using STRICT keys
    ...
    switch(key) {
        ...
        case 'endurance': 
            return data.filter(x => checkSport(x, 'BIKE')).map(x => {
                if (x._pwr > 0 && x._hr > 0) 
                    return { val: x._pwr / x._hr, date: x.date, dateStr: x.date.toISOString().split('T')[0], name: x.actualName, breakdown: `Pwr:${Math.round(x._pwr)} / HR:${Math.round(x._hr)}` };
            }).filter(Boolean);

        case 'strength': 
            return data.filter(x => checkSport(x, 'BIKE')).map(x => {
                if (x._pwr > 0 && x._cad > 0) 
                    return { val: x._pwr / x._cad, date: x.date, dateStr: x.date.toISOString().split('T')[0], name: x.actualName, breakdown: `Pwr:${Math.round(x._pwr)} / RPM:${Math.round(x._cad)}` };
            }).filter(Boolean);
        ...
        case 'run': 
            return data.filter(x => checkSport(x, 'RUN')).map(x => {
                if (x._spd > 0 && x._hr > 0) 
                    return { val: (x._spd * 60) / x._hr, date: x.date, dateStr: x.date.toISOString().split('T')[0], name: x.actualName, breakdown: `Pace:${Math.round(x._spd*60)}m/m / HR:${Math.round(x._hr)}` };
            }).filter(Boolean);

        case 'mechanical': 
            return data.filter(x => checkSport(x, 'RUN')).map(x => {
                if (x._spd > 0 && x._pwr > 0) 
                    return { val: (x._spd * 100) / x._pwr, date: x.date, dateStr: x.date.toISOString().split('T')[0], name: x.actualName, breakdown: `Spd:${x._spd.toFixed(1)} / Pwr:${Math.round(x._pwr)}` };
            }).filter(Boolean);
        ...
        case 'swim': 
            return data.filter(x => checkSport(x, 'SWIM')).map(x => {
                if (x._spd > 0 && x._hr > 0) 
                    return { val: (x._spd * 60) / x._hr, date: x.date, dateStr: x.date.toISOString().split('T')[0], name: x.actualName, breakdown: `Spd:${(x._spd*60).toFixed(1)}m/m / HR:${Math.round(x._hr)}` };
            }).filter(Boolean);
        
        // GENERAL
        case 'vo2max': 
            return data.map(x => {
                if (x._vo2 > 0) return { val: x._vo2, date: x.date, dateStr: x.date.toISOString().split('T')[0], name: "VO2 Est", breakdown: `Score: ${x._vo2}` };
            }).filter(Boolean);

        case 'anaerobic': 
            return data.map(x => {
                if (x._ana > 0.5) return { val: x._ana, date: x.date, dateStr: x.date.toISOString().split('T')[0], name: x.actualName, breakdown: `Anaerobic: ${x._ana}` };
            }).filter(Boolean);
...
export const calculateSubjectiveEfficiency = (allData, sportMode) => {
    return allData.map(d => {
        ...
        if (sportMode === 'bike' && checkSport(d, 'BIKE') && d._pwr > 0) {
            val = d._pwr / rpe; breakdown = `${Math.round(d._pwr)}W / ${rpe} RPE`; match = true;
        }
        else if (sportMode === 'run' && checkSport(d, 'RUN') && d._spd > 0) {
            val = d._spd / rpe; breakdown = `${d._spd.toFixed(2)} m/s / ${rpe} RPE`; match = true;
        }
