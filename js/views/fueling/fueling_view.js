                ...
                <div id="fuel-active-view" class="hidden">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div id="card-eat" class="bg-slate-800 border-2 border-slate-700 rounded-2xl p-6 flex flex-col items-center justify-center">
                            ...
                            <div id="custom-fuel-input-area" class="w-full mt-3 pt-3 border-t border-slate-700/50 hidden">
                            ...
                            <button id="btn-show-custom-fuel" class="text-[10px] text-slate-500 mt-2 underline">Log Custom Amount</button>
                        </div>
                    </div>
                </div>
                ...
                <div id="fuel-config-view" class="bg-slate-800 rounded-xl p-6 border border-slate-700">
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                        <div>
                            ...
                            <input type="number" id="input-drink-int" value="${state.drinkInterval}" class="w-full bg-slate-900 border-slate-600 rounded p-2 text-white">
                        </div>
                        <div>
                            <label class="block text-[10px] text-slate-400 uppercase font-bold mb-1">Eat (min)</label>
                            <input type="number" id="input-eat-int" value="${state.eatInterval}" class="w-full bg-slate-900 border-slate-600 rounded p-2 text-white">
                        </div>
                        <div>
                            <label class="block text-[10px] text-blue-400 uppercase font-bold mb-1">Bottle (g)</label>
                            <input type="number" id="input-bottle-carbs" value="${state.carbsPerBottle}" class="w-full bg-slate-900 border-slate-600 rounded p-2 text-white">
                        </div>
                        <div>
                            <label class="block text-[10px] text-emerald-400 uppercase font-bold mb-1">Target (g/hr)</label>
                            <input type="number" id="input-target-hourly" value="${state.targetHourlyCarbs}" class="w-full bg-slate-900 border-slate-600 rounded p-2 text-white">
                        </div>
                    </div>

                    <div class="bg-slate-900/50 p-4 rounded-lg border border-slate-700">
                        <div class="text-[10px] text-slate-500 uppercase font-bold mb-2 flex justify-between">
                            <span>Active Menu Items</span>
                            <span class="text-[10px] italic opacity-60">Check to show in HUD</span>
                        </div>
                        <div id="fuel-library-editor" class="space-y-2 max-h-[250px] overflow-y-auto pr-2 custom-scrollbar mb-3">
                            ${this.renderFuelEditor(state.fuelMenu)}
                        </div>
