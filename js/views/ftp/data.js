...
export const FTPData = {
    ...
    parseRunningMarkdown(md) {
        ...
        // Deduplication: Group by roughly matching distance (to merge 1600m/1mi if logic missed it)
        const uniqueData = {};
        rawData.forEach(pt => {
            const key = pt.x.toFixed(2); // precision to merge slightly different calc
            if (!uniqueData[key]) {
                uniqueData[key] = pt;
            } else {
