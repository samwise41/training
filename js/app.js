    ...
    const App = {
        ...
        async loadData() {
            try {
                ...
                this.allData = Parser.parseTrainingLog(rawLog); // Keeps compatibility with other views
        ...
        renderCurrentView(viewName) {
            ...
            setTimeout(() => {
                ...
                try {
                    if (view === 'trends') {
                    ...
                    else if (view === 'zones') { content.innerHTML = renderZones(this.garminData); }
                    else if (view === 'ftp') { content.innerHTML = renderFTP(this.garminData); }
                    else if (view === 'roadmap') { content.innerHTML = renderRoadmap(this.garminData, this.planMd); }
                    else if (view === 'readiness') { content.innerHTML = renderReadiness(this.garminData); }
                    else if (view === 'metrics') { content.innerHTML = renderMetrics(this.garminData); }
                    else if (view === 'plan') {
                        ...
                        // FIX: Pass 3 arguments: Planned (JSON), Actual (JSON), and Plan Text (Markdown)
                        const html = this.getStatsBar() + renderDashboard(this.plannedData, this.allData, this.planMd);
