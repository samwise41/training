...
export const renderSummaryTable = (allData) => {
    ...
    groups.forEach(group => {
        ...
        group.keys.forEach(key => {
            ...
            else fullData = extractMetricData(allData, key);
            ...
            if (recent.length > 0) {
                ...
                statusHtml = isGood 
                    ? '<span class="text-emerald-400 font-bold text-[10px] bg-emerald-900/30 px-1.5 py-0.5 rounded">✅ On Target</span>'
                    : '<span class="text-red-400 font-bold text-[10px] bg-red-900/30 px-1.5 py-0.5 rounded">⚠️ Off Target</span>';
            }
