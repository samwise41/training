...
export const FTPCharts = {
    ...
    renderSvgCurve(data, options) {
        ...
        // Added ID hooks for interaction
        return `
        <div class="relative w-full h-full group select-none">
            ...
            <div id="${options.containerId}-tooltip" class="absolute hidden bg-slate-900/95 border border-slate-700 rounded shadow-xl p-3 z-50 min-w-[140px] pointer-events-none"></div>
        </div>`;
    },

    // --- 2. SVG INTERACTION LOGIC (Restored) ---
    setupSvgInteractions(containerId, data, options) {
        ...
        // Pre-calculate pixel positions for performance
        ...
        const updateUI = (closest) => {
            ...
            tooltip.innerHTML = `
                ...
                <div class="flex flex-col gap-1">
                    <div class="flex justify-between text-xs"><span style="color:${colorAll}">All Time</span><span class="font-mono text-white font-bold">${valAll}</span></div>
                    <div class="flex justify-between text-xs"><span style="color:${color6w}">6 Weeks</span><span class="font-mono text-white font-bold">${val6w}</span></div>
                </div>`;
        ...
        svg.addEventListener('mousemove', (e) => {
            // if (isLocked) return; // Optional: Unlock on hover? usually locking is for clicking.
            const rect = svg.getBoundingClientRect();
            ...
            if (closest && minDist < 50) updateUI(closest);
        });
    ...
    // --- 3. CHART.JS RENDERERS (With Custom Tooltips) ---
    renderBikeHistory(canvasId, data, color) {
        ...
        new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [
                    { label: 'FTP (w)', data: data.map(d => d.ftp), borderColor: color, backgroundColor: color + '20', borderWidth: 2, pointRadius: 3, pointHoverRadius: 5, yAxisID: 'y' },
                    { label: 'W/kg', data: data.map(d => d.wkg), borderColor: '#34d399', borderWidth: 1.5, borderDash: [3, 3], pointRadius: 3, pointHoverRadius: 5, yAxisID: 'y1' }
                ]
            },
    ...
    renderRunHistory(canvasId, data, color) {
        ...
        new Chart(ctx, {
            ...
            data: {
                labels: data.map(d => d.date),
                datasets: [
                    { label: 'Pace', data: data.map(d => d.pace), borderColor: color, backgroundColor: color + '20', borderWidth: 2, pointRadius: 3, pointHoverRadius: 5, yAxisID: 'y' },
                    { label: 'LTHR', data: data.map(d => d.lthr), borderColor: '#ef4444', borderWidth: 1, borderDash: [3, 3], pointRadius: 3, pointHoverRadius: 5, yAxisID: 'y1' }
                ]
            },
            options: this._getCommonOptions({
    ...
    _getCommonOptions(scales) {
        return {
            ...
            plugins: {
                ...
                // Use Custom External Tooltip to match App Theme
                tooltip: {
                    enabled: false,
                    external: (context) => {
                        ...
                        if (!tooltipEl) {
                            ...
                            tooltipEl.className = 'bg-slate-900/95 border border-slate-700 rounded shadow-xl p-3 z-50 min-w-[140px] pointer-events-none absolute transition-opacity duration-200';
                        ...
                        if (tooltipModel.body) {
                            ...
                            let innerHtml = `<div class="border-b border-slate-700 pb-1 mb-2 text-[10px] font-bold text-slate-300 uppercase tracking-wider">${title}</div>`;
                            
                            bodyLines.forEach((body, i) => {
                                const colors = tooltipModel.labelColors[i];
                                const style = `background:${colors.borderColor}`;
                                // Format Value (Check if it looks like Pace in seconds)
                                let val = body[0].split(':')[1];
                                // Hacky check if we need to format pace
                                if(context.chart.scales.y.options.reverse) {
                                    // This is likely the run chart
                                     const raw = context.tooltip.dataPoints[i].raw;
                                     if(context.tooltip.dataPoints[i].datasetIndex === 0) val = formatPaceSec(raw);
                                }

                                innerHtml += `<div class="flex items-center justify-between text-xs gap-3 mb-1">
                                    <div class="flex items-center gap-1"><span class="w-2 h-2 rounded-full inline-block" style="${style}"></span><span class="text-slate-400">${body[0].split(':')[0]}</span></div>
