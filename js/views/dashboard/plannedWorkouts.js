...
function generateGroupedCardsHTML(groupedData) {
    ...
    groupedData.forEach(day => {
        ...
        // --- Outer Card Border (Overall Status) ---
        let cardBorderClass = "border border-slate-700 bg-slate-800"; 
        
        if (isDayComplete) {
            // Green Ring only if ALL items are done
        ...
        const rowsHtml = day.workouts.map(w => {
            ...
            if (w.status === 'COMPLETED' || w.status === 'UNPLANNED') {
                ...
                borderClass = "border border-emerald-500/60"; // Green Border
            } else if (w.status === 'MISSED') {
                statusColorClass = "text-red-400";
                borderClass = "border border-red-500/60"; // Red Border
            } else if (w.status === 'PLANNED') {
                // Keep planned items neutral or blue-ish depending on preference
                // Standardizing on Slate/Blue mix for Planned
                statusColorClass = "text-blue-400"; 
                borderClass = "border border-blue-500/40"; // Blue Border
            } else if (w.status === 'REST') {
                borderClass = "border border-slate-700/50"; // Faint border for Rest
            }

            const sportColorClass = `icon-${sportType.toLowerCase()}`;

            // --- THE ROW HTML ---
            // Uses 'rounded-md p-3 mb-2' to create the box effect from your screenshot
            return `
                <div class="flex items-start gap-4 p-3 mb-2 rounded-md ${borderClass} bg-slate-800/30 hover:bg-slate-700/30 transition-colors">
