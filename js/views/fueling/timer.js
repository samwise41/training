...
export const FuelTimer = {
    state: {
        ...
        // FUEL LIBRARY (Will load from JSON)
        fuelMenu: [],
    ...
    async init() {
        // 1. Try loading from your new JSON file
        let library = await DataManager.fetchJSON('fuelLibrary');
        
        // 2. Fallback if the file is missing or empty
    ...
    renderFuelButtons() {
        if (!this.state.fuelMenu || this.state.fuelMenu.length === 0) return '<div class="col-span-2 text-xs text-slate-500 italic">No items found.</div>';
