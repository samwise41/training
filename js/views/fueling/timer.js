...
export const FuelTimer = {
    ...
    async init() {
        ...
        if (!library || library.length === 0) {
            library = [
                { id: 'gel', label: 'Gel', carbs: 22, icon: 'fa-bolt', active: false },
                { id: 'chews', label: 'Chews', carbs: 24, icon: 'fa-candy-cane', active: false },
                { id: 'bar', label: 'Bar', carbs: 40, icon: 'fa-cookie-bite', active: false }
            ];
        }
    ...
    refreshLogUI() {
        ...
        if (logContainer) logContainer.innerHTML = FuelView.renderHistoryLog(this.state.consumptionLog);
    },
